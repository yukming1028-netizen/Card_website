...
### 後端 Dockerfile（`backend/Dockerfile.render`）
...
### 前端 Dockerfile（`frontend/Dockerfile.render`）
...
```bash
REACT_APP_API_URL=https://你的後端域名.onrender.com/api
```
